...
def build_graph():
    ...
    x_fold = tf.reshape(x, (-1, FLAGS.timestep_size * FLAGS.input_size))
    # nn
    W1 = tf.Variable(tf.truncated_normal([FLAGS.input_size * FLAGS.timestep_size, FLAGS.hidden_dim], stddev=0.1), dtype=tf.float32)
    ...
    hidden_layer = tf.nn.sigmoid(tf.matmul(x_fold, W1) + bias1)
...
def main(argv=None):
    ...
    print(test_accuracy)
    np.savetxt('./train_results/MLP/train_loss' + str(FLAGS.batch_size) + '_' + str(FLAGS.hidden_dim) + '_' + str(
        FLAGS.lr) + '.txt', train_loss)
    np.savetxt('./train_results/MLP/test_accuracy' + str(FLAGS.batch_size) + '_' + str(FLAGS.hidden_dim) + '_' + str(
